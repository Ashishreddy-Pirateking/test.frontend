...
import { RASA_FEEDBACK } from "../data/legacyData";
...
const getFeedbackComment = (emotionCode, score) => {
  const comments = Array.isArray(RASA_FEEDBACK[emotionCode]) ? RASA_FEEDBACK[emotionCode] : RASA_FEEDBACK.DEFAULT;
  const idx = getScoreBandIndex(score);
  return comments[idx] || comments[comments.length - 1] || "No feedback available.";
};
